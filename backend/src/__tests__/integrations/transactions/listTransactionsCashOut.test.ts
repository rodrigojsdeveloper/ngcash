...
describe('Tests for transaction routes', () => {
    ...
    test('Must be able to list transactions cash-out', async () => {
        ...
        const response = await request(app).get('/transactions/cash-out').set('Authorization', `Bearer ${ token }`)
    ...
    test('Must be able to prevent listing transactions cash-out without token', async () => {
        ...
        const response = await request(app).get('/transactions/cash-out').send(transaction)
